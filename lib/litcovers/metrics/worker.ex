defmodule Litcovers.Metrics.Worker do
  use GenServer, restart: :temporary

  @registry Litcovers.Metrics.Registry
  ...
  defp upsert!(path, counter) do
    import Ecto.Query
    date = Date.utc_today()
    query = from(m in Litcovers.Metrics.Metric, update: [inc: [counter: ^counter]])

    Litcovers.Repo.insert!(
      %Litcovers.Metrics.Metric{date: date, path: path, counter: counter},
      on_conflict: query,
