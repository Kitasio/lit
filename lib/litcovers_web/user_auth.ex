defmodule LitcoversWeb.UserAuth do
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_litcovers_web_user_remember_me"
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      LitcoversWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  the current_user:
      ...
      defmodule LitcoversWeb.PageLive do
        use LitcoversWeb, :live_view

        on_mount {LitcoversWeb.UserAuth, :mount_current_user}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{LitcoversWeb.UserAuth, :ensure_authenticated}] do
  ...
  def enabled_user(conn, _opts) do
    if conn.assigns[:current_user].enabled do
      conn
    else
      conn
  ...
  defp signed_in_path(conn), do: ~p"/#{conn.assigns.locale}/images"
end
